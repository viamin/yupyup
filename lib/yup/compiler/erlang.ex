defmodule Yup.Compiler.Erlang do
  ...
  defp expr(%RecordConstruction{name: _name, fields: fields} = node) do
    line = line(node)

    field_asts =
      Enum.map(fields, fn {field_name, value, _field_loc} ->
        {:map_field_assoc, line, {:atom, line, String.to_atom(field_name)}, expr(value)}
      end)

    {:map, line, field_asts}
  end

  defp expr(%FieldAccess{record: record, field: field} = node) do
    remote_call(
      line(node),
      :maps,
      :get,
      [{:atom, line(node), String.to_atom(field)}, expr(record)]
    )
  end
